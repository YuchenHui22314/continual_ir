...
Inputs: figures/bucket_runs_eval.json (B5 offline eval),
        figures/analysis/ewc_forgetting_predictor_infonce6e6.json (overlaps; optional).
"""
...
EVAL_JSON = f"{FIG_DIR}/bucket_infonce6e6_runs_eval.json"
EWC_JSON  = f"{FIG_DIR}/analysis/ewc_forgetting_predictor_infonce6e6.json"
...
for i, tb in enumerate(BUCKETS):
    e = D[f"bucket_infonce6e6_{tb}"]["470"]["topiocqa"]
    for j, eb in enumerate(BUCKETS):
...
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_transfer_matrix.png", bbox_inches="tight")
plt.close(fig)
...
for i, b in enumerate(BUCKETS):
    ys = [D[f"bucket_infonce6e6_{b}"][str(s)]["msmarco"] for s in STEPS]
...
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_forgetting_curves.png", bbox_inches="tight")
plt.close(fig)
print("saved bucket_forgetting_curves.png")

# ── 3. in-length gain + MSMARCO drop vs length ───────────────────────────────
gain = [D[f"bucket_infonce6e6_{b}"]["470"]["topiocqa"][b]["NDCG@10"]
        - ZS["topiocqa"][b]["NDCG@10"] for b in BUCKETS]
drop = [ZS["msmarco"] - D[f"bucket_infonce6e6_{b}"]["470"]["msmarco"] for b in BUCKETS]
...
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_inlength_gain.png", bbox_inches="tight")
plt.close(fig)
...
try:
    ...
    fig.tight_layout()
    fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_fisher_overlap.png", bbox_inches="tight")
    plt.close(fig)
...
x = np.arange(len(BUCKETS))
drop470 = [(ZS["msmarco"] - D[f"bucket_infonce6e6_{b}"]["470"]["msmarco"]) * 100 for b in BUCKETS]
drop47  = [(ZS["msmarco"] - D[f"bucket_infonce6e6_{b}"]["47"]["msmarco"]) * 100 for b in BUCKETS]
fig, ax = plt.subplots(figsize=(8.5, 5))
...
ax.legend(frameon=False)
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_dose_response.png", bbox_inches="tight")
plt.close(fig)
...
try:
    ...
    fig.tight_layout()
    fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_overlap_vs_drop.png", bbox_inches="tight")
    plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_transfer_delta.png", bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_generalization_profiles.png", bbox_inches="tight")
plt.close(fig)
print("saved bucket_generalization_profiles.png")

# ── 8b. generalization profiles — ALL 13 buckets (saturation made explicit) ──
# Same y-axis as fig 8 (Delta TopiOCQA NDCG@10 vs zero-shot, x100) but every
# training bucket is drawn, coloured by length via a colorbar instead of a
# 13-entry legend. The collapse of all turn>=3 lines onto one band IS the
# "profiles saturate with training length" evidence; only turn_1 (and mildly
# turn_2) sit apart.
fig, ax = plt.subplots(figsize=(9.5, 5.6))
cmap = plt.get_cmap("viridis")
for i, b in enumerate(BUCKETS):
    ax.plot(range(len(LABELS)), Md[i, :], "-o", ms=3, lw=1.6,
            color=cmap(i / (len(BUCKETS) - 1)))
ax.axhline(0, color="gray", lw=1, ls=":")
ax.set_xticks(range(len(LABELS)), LABELS)
ax.set_xlabel("eval turn length (TopiOCQA valid subset)")
ax.set_ylabel(r"$\Delta$ TopiOCQA NDCG@10 vs zero-shot ($\times$100)")
sm = plt.cm.ScalarMappable(cmap=cmap,
                           norm=plt.Normalize(vmin=0, vmax=len(BUCKETS) - 1))
cb = fig.colorbar(sm, ax=ax, ticks=[0, len(BUCKETS) - 1])
cb.ax.set_yticklabels(["train\nturn 1", "train\nturn 15+"])
cb.set_label("training bucket (conversation length)")
fig.tight_layout()
fig.savefig(f"{FIG_DIR}/bucket_infonce6e6_generalization_profiles_all.png", bbox_inches="tight")
plt.close(fig)
print("saved bucket_generalization_profiles_all.png")
