...
from data import Topiocqa, MSMARCODataset
...
def get_infinite_iterator(dataloader):
    """Wrapper to make a dataloader infinite for Experience Replay"""
    while True:
        for batch in dataloader:
            yield batch


def merge_query_tokens(args, topiocqa_batch, msmarco_batch=None):
    """
    Merge query input_ids / masks BEFORE encoding.
    """
    topiocqa_query_tokens = topiocqa_batch["complex_query"]
    topiocqa_query_mask = topiocqa_batch["complex_query_mask"]
    #B_topi = topiocqa_query_tokens.size(0)

    if msmarco_batch is None:
        return topiocqa_query_tokens.to(args.device), topiocqa_query_mask.to(args.device)#, B_topi

    msmarco_query_tokens = msmarco_batch["complex_query"]
    msmarco_query_mask = msmarco_batch["complex_query_mask"]

    merged_tokens = torch.cat([topiocqa_query_tokens, msmarco_query_tokens], dim=0).to(args.device)
    merged_mask = torch.cat([topiocqa_query_mask, msmarco_query_mask], dim=0).to(args.device)
    return merged_tokens, merged_mask #, B_topi

def get_doc_embeddings(args, batch, passage_encoder, embedding_table=None):
    """
    get positive negative oracle embeddings for a batch.
    Handles both on-the-fly encoding and pre-computed embedding lookup.
    """
    if batch is None:
        return None, None, None

    sample_ids = batch['sample_ids']  # list of sample ids in the batch
    
    pos_doc_embs = None
    neg_doc_embs = None
    oracle_utt_embs = None

    # ###############################
    # ### calcualte (get) embeddings. 
    # ###############################

    # 2. Document/Oracle Embeddings (On-the-fly OR Lookup)
    ...
    if embedding_table is None:
        ...
        # --- Lookup from Table ---
        # Note: keys in table must match sample_ids (str or int)
        pos_doc_embs = torch.stack(
            [embedding_table[sid]["pos"] for sid in sample_ids],
            dim=0
        ).to(args.device) # [B, dim]

        if embedding_table[sample_ids[0]]["neg"] is not None and args.negative_type != "none":
            neg_doc_embs = torch.stack(
                [embedding_table[sid]["neg"] for sid in sample_ids],
                dim=0
            ).to(args.device)

        if "kd" in args.loss_type:
             # Check if oracle exists (MSMARCO might not have it)
            if embedding_table[sample_ids[0]]["oracle"] is not None:
                oracle_utt_embs = torch.stack(
                    [embedding_table[sid]["oracle"] for sid in sample_ids],
                    dim=0
                ).to(args.device)
            else:
                 # Fallback if KD is requested but oracle embeddings missing (e.g. for MSMARCO)
                 # We create zeros or handle this logic outside. 
                 oracle_utt_embs = None
    
    # end if: pre-encoded pos neg oracle embeddings

    return pos_doc_embs, neg_doc_embs, oracle_utt_embs 
...
def train(args):
    ...
    # TopiOCQA embeddings
    sample_emb_table = None
    if args.pos_neg_embedding_file is not None:
        logger.info(f"Loading Topiocqa embeddings from {args.pos_neg_embedding_file}")
    ...
    # MSMARCO embeddings (Experience Replay)
    msmarco_emb_table = None
    if args.msmarco_embedding_file is not None and args.experience_replay_batch_size > 0:
        logger.info(f"Loading MSMARCO embeddings from {args.msmarco_embedding_file}")
        msmarco_emb_table = torch.load(args.msmarco_embedding_file, map_location="cpu")
    ...
    # If any dataset needs on-the-fly encoding
    passage_encoder = None
    if sample_emb_table is None or (msmarco_emb_table is None and args.experience_replay_batch_size > 0):
    ...
    # 3. Prepare MSMARCO Data (Experience Replay)
    msmarco_iterator = None
    if args.experience_replay_batch_size > 0:
        logger.info("Initializing MSMARCO Experience Replay Dataset...")
        msmarco_dataset = MSMARCODataset(args, query_tokenizer, args.msmarco_data_file)
        
        if args.n_gpu > 1:
            # Important: Use DistributedSampler so different GPUs get different replay samples
            ms_sampler = DistributedSampler(msmarco_dataset, shuffle=True, drop_last=True)
        else:
            ms_sampler = RandomSampler(msmarco_dataset)
        ...
        msmarco_iterator = get_infinite_iterator(msmarco_loader)
    ...
    for epoch in epoch_iterator:
        ...
        if args.n_gpu > 1:
            ...
            # We don't strictly need to set_epoch for msmarco as it's an infinite iterator, 
            # but usually good practice if we were iterating epoch-wise. 
            # Since we wrap it in get_infinite_iterator, the randomness comes from the sampler shuffling.
        ...
        for batch in tqdm(train_loader, desc="Step"):
            ...
            # --- A. get MSMARCO Batch (if enabled) ---
            if msmarco_iterator is not None:
                msmarco_batch = next(msmarco_iterator)
            else:
                msmarco_batch = None

            # --- B. Merge Query Batches and forward ---
            merged_query_tokens, merged_query_mask = merge_query_tokens(args, batch, msmarco_batch)
            merged_query_embs = query_encoder(merged_query_tokens, attention_mask=merged_query_mask)
            
            # --- C. get doc embeddings ---
            topiocqa_pos_doc_embs, topiocqa_neg_doc_embs, topiocqa_oracle_utt_embs = get_doc_embeddings(
                args,
                batch,
                passage_encoder,
                sample_emb_table)
            msmarco_pos_doc_embs, msmarco_neg_doc_embs, msmarco_oracle_utt_embs = get_doc_embeddings(
                args, 
                msmarco_batch, 
                passage_encoder, 
                msmarco_emb_table)

            # merge pos, neg, oracle embeddings if any
            if msmarco_pos_doc_embs is not None:
                merged_pos_doc_embs = torch.cat([topiocqa_pos_doc_embs, msmarco_pos_doc_embs], dim=0)
            else:
                merged_pos_doc_embs = topiocqa_pos_doc_embs

            if topiocqa_neg_doc_embs is not None and msmarco_neg_doc_embs is not None:
                merged_neg_doc_embs = torch.cat([topiocqa_neg_doc_embs, msmarco_neg_doc_embs], dim=0)   
            else:
                merged_neg_doc_embs = None
            
            # wrong kd loss logic, to be modified later: TODO
            if topiocqa_oracle_utt_embs is not None and msmarco_oracle_utt_embs is not None:
                merged_oracle_utt_embs = torch.cat([topiocqa_oracle_utt_embs, msmarco_oracle_utt_embs], dim=0)
            else:
                merged_oracle_utt_embs = topiocqa_oracle_utt_embs
            

            # --- Loss Calculation ---
            
            if args.negative_type == "none":
                assert merged_neg_doc_embs is None, "neg_doc_embs should be None when negative_type is 'none'."

            # 1. Ranking Loss (Applied to combined batch)
            ranking_loss = cal_ranking_loss(merged_query_embs, merged_pos_doc_embs, merged_neg_doc_embs)

            # 2. KD Loss
            kd_loss = torch.tensor(0.0).to(args.device)
            if "kd" in args.loss_type and topiocqa_oracle_utt_embs is not None:
                # If we have ER, we only have oracle for the TopiOCQA part (usually)
                kd_loss = cal_kd_loss(merged_query_embs, merged_oracle_utt_embs)
            ...
            if is_main_process and cur_step % args.log_print_steps == 0:
                ...
                if args.save_to_wandb:
                    wandb.log(
                        {
                            "train/loss": loss_val,
                            "train/ranking_loss": ranking_loss_val,
                            "train/kd_loss": kd_loss_val,
...
def get_args():
    ...
    # --- New Arguments for Experience Replay ---
    parser.add_argument("--experience_replay_batch_size", type=int, default=0, help="Batch size for MSMARCO experience replay. 0 to disable.")
    parser.add_argument("--msmarco_data_file", type=str, default=None, help="Path to MSMARCO training jsonl.")
    parser.add_argument("--msmarco_embedding_file", type=str, default=None, help="Path to MSMARCO pre-computed embeddings (dictionary).")
