...
from data_format import padding_seq_to_same_length, topiocqa_normal, test_cast
import os
from os import path
...
def get_test_query_embedding(args):
    ...
    test_dataset = test_cast(args, tokenizer)
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, disable=args.disable_tqdm):
            ...
            query_embs = model(bt_concat, bt_concat_mask)
...
def output_test_res(query_embedding2id,
    ...
    if "cast21" in args.original_file:
...
def get_args():
    ...
    parser.add_argument("--original_file", type=str, default="datasets/cast/cast20_eval_topics.json")
    parser.add_argument("--passage_embeddings_dir_path", type=str, default="index/cast1920/ance")
    parser.add_argument("--pretrained_encoder_path", type=str, default="") # trained model
    parser.add_argument("--qrel_output_path", type=str, default="../output/cast20")
    parser.add_argument("--output_trec_file", type=str, default="")
    parser.add_argument("--trec_gold_qrel_file_path", type=str, default="datasets/cast/cast20_qrels.tsv")

    parser.add_argument("--collate_fn_type", type=str, default="flat_concat_for_test", help="To control how to organize the batch data.")
    ...
    if "cast21" in args.original_file:
        args.passage_block_num = 9
    if "cast19" in args.original_file:
