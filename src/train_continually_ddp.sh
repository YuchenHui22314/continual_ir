#!/usr/bin/env bash
set -e # exit on error

############################
# Conda environment
############################
source ~/miniconda3/etc/profile.d/conda.sh
conda activate /data/rech/huiyuche/envs/trec_ikat

############################
# Project root
############################
cd /data/rech/huiyuche/continual_ir/src

############################
# Environment
############################
export CUDA_VISIBLE_DEVICES=0,1,2,3
export CUDA_LAUNCH_BLOCKING=1
export TOKENIZERS_PARALLELISM=false

############################
# DDP config
############################
NUM_GPUS=4
MASTER_PORT=29501   # you can change this port if needed.

############################
# Paths
############################
ENCODER_PATH="/data/rech/huiyuche/huggingface/models--castorini--ance-msmarco-passage/snapshots/6d7e7d6b6c59dd691671f280bc74edb4297f8234"
TRAIN_FILE="/part/01/Tmp/yuchen/topics/topiocqa/topiocqa_train_oracle.jsonl"
#OUTPUT_DIR="/data/rech/huiyuche/huggingface/continual_ir/topiocqa_120bs_beir"
OUTPUT_DIR="/data/rech/huiyuche/huggingface/continual_ir"
POS_NEG_EMB="/data/rech/huiyuche/TREC_iKAT_2024/data/embeddings/topiocqa_pos_neg_docs_ance/embeddings.pt"
BEIR_EMB_DIR="/part/01/Tmp/yuchen/beir/embeddings/ance/"
BEIR_QUERY_CORPUS_PATH="/part/01/Tmp/yuchen/beir"
# msmarco
MSMARCO_DATA_FILE="/part/01/Tmp/yuchen/topics/msmarco/msmarco_train.jsonl"
MSMARCO_EMBEDDING_FILE="/data/rech/huiyuche/TREC_iKAT_2024/data/embeddings/msmarco_pos_neg_docs_ance/embeddings_multi_GPU.pt"


LOG_PATH="/data/rech/huiyuche/TREC_iKAT_2024/logs/train_log_topiocqa.txt"

############################
# Launch grid search
############################

TOTAL_BATCH=120

TRAIN_BATCH_LIST=(110 100 90 80 70 60 50 40)

for TRAIN_BS in "${TRAIN_BATCH_LIST[@]}"; do
    REPLAY_BS=$((TOTAL_BATCH - TRAIN_BS))

    WANDB_NAME="continual_${TRAIN_BS}_${REPLAY_BS}_no_negative"
    EXP_OUTPUT_DIR="${OUTPUT_DIR}/${WANDB_NAME}"

    echo "========================================"
    echo "Running experiment:"
    echo "  Train batch  = ${TRAIN_BS}"
    echo "  Replay batch = ${REPLAY_BS}"
    echo "  WandB name   = ${WANDB_NAME}"
    echo "  Output dir   = ${EXP_OUTPUT_DIR}"
    echo "========================================"

    torchrun \
      --nproc_per_node ${NUM_GPUS} \
      --master_port ${MASTER_PORT} \
      train_continually_ddp.py \
      --n_gpu ${NUM_GPUS} \
      --pretrained_encoder_path "${ENCODER_PATH}" \
      --training_data_file "${TRAIN_FILE}" \
      --model_output_path "${EXP_OUTPUT_DIR}" \
      --pos_neg_embedding_file "${POS_NEG_EMB}" \
      --loss_type ranking \
      --negative_type "none" \
      --num_train_epochs 25 \
      --per_gpu_train_batch_size ${TRAIN_BS} \
      --learning_rate 1e-5 \
      --weight_decay 0.00 \
      --warmup_ratio 0.06 \
      --max_query_length 64 \
      --max_doc_length 512 \
      --max_concat_length 512 \
      --log_print_ratio 0.1 \
      --save_to_wandb \
      --wandb_project "continual_ir" \
      --wandb_name "${WANDB_NAME}" \
      --activate_eval_while_training \
      --beir_embedding_dir "${BEIR_EMB_DIR}" \
      --eval_batch_size 64 \
      --beir_datasets climate-fever msmarco \
      --beir_query_corpus_path "${BEIR_QUERY_CORPUS_PATH}" \
      --use_data_percent 1.0 \
      --experience_replay_batch_size ${REPLAY_BS} \
      --msmarco_data_file "${MSMARCO_DATA_FILE}" \
      --msmarco_embedding_file "${MSMARCO_EMBEDDING_FILE}" \
      2>&1 | tee -a $LOG_PATH

done